...
const count = (target, input) => {
  // Solution code here...
  return input.map(arr => arr.filter(num => num === target)).reduce((acc, curr) => {
    acc += curr.length
    return acc
  }, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  return input.map(arr => arr === [] ? []
    : arr.filter(ele => typeof ele === 'number' && ele % 5 === 0)
